...
configure_apt_proxy() {
  ...
  if [[ -n "$http_proxy_wsl" || -n "$https_proxy_wsl" ]]; then
    ...
    echo "Acquire::http::Proxy \"$http_proxy_wsl\";" | tee /etc/apt/apt.conf
    echo "Acquire::https::Proxy \"$https_proxy_wsl\";" | tee -a /etc/apt/apt.conf
  else
...
install_ansible() {
  ...
  apt-get install -yqq ansible jq >/dev/null 2>&1
  if [ $? -ne 0 ]; then
