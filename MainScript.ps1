...
try {
  ...
  switch ($exitCode) {
    ...
    2 {
      Write-Host "⚠️ cloud-init finished with 'degraded' status (code 2), but no blocking errors." -ForegroundColor Yellow
    }
    default {
      $message = "cloud-init failed with exit code: $exitCode"
